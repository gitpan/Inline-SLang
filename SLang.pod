...
It should be possible to also bind variables, but this
...
Where possible a I<perlish> interface is retained.
...
C<[3,2]> - or C<[24]> say - then expect weird behaviour
at the very least. 
...
(i.e. the L<Perl Data Language|PDL> support that is not yet
available). This routine is more useful for arrays of
...
the S-Lang C<eval()> function (perhaps this should be moved
into C<sl_eval>?):
...
a large array or complicated structure).
...
I do not have the time to support anything but a recent version
of S-Lang - defined as 1.4.7 or higher - on Solaris and Linux
machines. This will hopefully extend to Mac OS X and Alpha
machines in the near future.

The code is I<NOT> guaranteed to work with an earlier version;
in fact F<Makefile.PL> should have screamed at you and gone off
in a sulk if you tried to build againt an earlier version.

It is intended to support Perl versions >= 5.6.0 only (at least the
release versions rather than the development releases). However note
that the development is made using version 5.8.0, and so there
may be subtle/hidden issues with the 5.6.x series of Perl.
It is not
...
This is I<alpha> release code. So, by design,
