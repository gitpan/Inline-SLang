...
=head1 DESCRIPTION
 ...
 # is not finalised.
 #
 # As of version 0.24 the PDL support is turned back on by default for
 # Solaris and Linux systems. There are still problems on OS-X machines
 # (when converting S-Lang arrays to piddles), but it no longer crashes
