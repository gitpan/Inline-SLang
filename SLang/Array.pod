...
The C<SOME_UNNAMED_FUNCTION> is used to define how
S-Lang arrays are converted to Perl.

=head2 Things to remember when converting arrays
...
=head3 How to confuse the array conversion (by data type)
...
=head3 Helping the code out by telling it about the array
...
Unfortunately you can I<not> use this mechanism to
typecast the array. If you replace C<Int_Type> with C<String_Type> 
...
constructor call for the C<Array_Type> class.

=head2 The Array_Type Class
...
object: iseful but also liable to lead to crashes or
bizarre errors.
...
you are going to make te object unhappy, so take care.
