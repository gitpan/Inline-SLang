...
which would match the S-Lang language.
...
below to manipulate the contents of the array.
  ...
  my $assoc = Array_Type->new( "String_Type" );
  $$assoc{foo} = "hello";
  $$assoc{baz} = "hi there";
...
returns C<Assoc_Type>.
  ...
  my $assoc = Array_Type->new( "Complex_Type" );
...
Returns the number of keys in the associative array.
  ...
  my $assoc = Array_Type->new( "Double_Type" );
...
rather than Perl's C<keys %$assoc>.
...
There is no advantage to using this method
rather than Perl's C<values %$assoc>.
...
There is no advantage to using this method
rather than Perl's C<exists $$assoc{$key}>.
...
There is no advantage to using this method
rather than Perl's C<delete $$assoc{$key}>.
