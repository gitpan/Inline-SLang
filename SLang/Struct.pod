...
associative arrays, in that the keys are fixed (no adding or
deleting) and when you loop through them the order is
set by the orer they are stored in the structure.
So, we therefore convert S-Lang structures into
C<Struct_Type> objects which behave like hash references
...
executed when the code is loaded; so if you call S-Lang's
...
For "named" structs the field names array is not needed
...
rather than Perl's C<keys %$struct>.
...
There is no advantage to using this method
rather than Perl's C<$$struct{$key}>.
...
not provided by the object but it's not hard to add (or to
emulate).
...
The output of the code is something like (depending on whether you keep
